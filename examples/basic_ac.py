...
bot_testing = [958457225096085534]
...
async def color_searcher(ctx: discord.AutocompleteContext):
    ...
    """
    print(f'user in basic allowed: {ctx.interaction.user.id in BASIC_ALLOWED}')
...
@bot.slash_command(name="ac_example", guild_ids=bot_testing)
...
@bot.slash_command(name="ac_basic_example", guild_ids=bot_testing)
...
bot.run("OTU5NzMxMjc1MDY3OTczNjMy.YkgJZg.848kVCV4EAweusY7TNfVYWtTUzs")